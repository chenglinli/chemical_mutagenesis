...
snv <- function(input_file = NULL, ref_file = NULL, parental_strain = NULL, mutant_strain = NULL,
    ...
    for(rd in read_depth){
        ...
        # Allowed genotypes : "0", "1".  variants with >= 2 alternative alleles were saved to snp_filter2_fail
        gt_ <- "\\.|0|1"
        ...
        vcf_filter3_1 <- vcf_filter2_split[rowSums((ref_freq <= var_thld | ref_freq >= ref_thld)) == dim(ref_freq)[2],]
        ...
        if(is.null(vcf_filter3_2)){
            ...
            codon_bias <- read.table(file = paste(ref_file,"dd_codon_bias.txt",sep=""), header = T, sep="\t", stringsAsFactors = F)
